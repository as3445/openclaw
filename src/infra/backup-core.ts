...
export async function uploadToS3(
  ...
  try {
    ...
    if (error?.name === "NoSuchBucket") {
      ...
      throw new S3Error(`Invalid S3 access key ID. Check your credentials.`, error.name, error);
    } else if (error?.name === "SignatureDoesNotMatch") {
      throw new S3Error(`Invalid S3 secret access key. Check your credentials.`, error.name, error);
    }
...
export async function performBackup(config: BackupConfig): Promise<{
  ...
  try {
    ...
    // Create tarball
    const _tarballResult = await createBackupTarball({
